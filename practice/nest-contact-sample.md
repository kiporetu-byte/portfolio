...
Modules は Nest アプリの構成の基本単位で、imports, controllers, providers, exports を持てる。
...
Controllers は受信リクエストを処理し、ルーティングは decorator で定義する。
...
Providers/Services はロジック担当で、Nest の依存注入で controller から呼ばれる。
...
と機能が増えてきたときに、どこに何があるか整理しやすい。公式でも feature module で関連機能をまとめる構成を推している。
...
Prismaで保存だけ作ると、Nestの感覚がかなり分かる。

公式では Nest CLI で新規作成する方法が案内されている。
